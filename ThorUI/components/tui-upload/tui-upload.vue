	...
	export default {
		...
		methods: {
			...
			uploadImage: function(index, url) {
				let _this = this;
				let site_id = uni.getStorageSync('site');
				return new Promise((resolve, reject) => {
					uni.uploadFile({
						url: `https://wx.mjk24.com/v1/upload/card/?site_id=${site_id}`,
						name: this.fileKeyName,
						header: {
							Authorization: "JWT " + uni.getStorageSync('efmsToken')
						},
						formData: {},
						filePath: url,
						success: function(res) {
							if (res.statusCode == 200) {
								...
								console.log('0224',d.content.results);
								_this.imageList[index] = d.content.results;
								_this.$set(_this.statusArr, index, "1")
								resolve(index)
							} else {
