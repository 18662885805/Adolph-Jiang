...
<script>
	export default {
		...
		props: {
			...
			site_id:{
				type: String,
				default:'04962358-3bce-41a2-b3b0-81500bba32e3'
			}
		},
		async created() {
			let _self = this;
		...
		methods: {
			upImage(paths) {
				...
				Promise.all(promises).then(function(data) {
					console.log('0225',data)
					uni.hideLoading();
					_self.upload_cache_list.push(data[0]['url']);
					_self.emit();
				}).catch(function(res) {
					uni.hideLoading();
				});
			...
			emit() {
				let _self = this;
				_self.$emit('change',{imgArr: _self.upload_cache_list} );
			}
		}
	}
	...
	const upload = function(options) {
		...
		const uploadTask = uni.uploadFile({
			url: `https://wx.mjk24.com/v1/upload/card/?site_id=${site_id}`,
			header:{Authorization: "JWT " + uni.getStorageSync('efmsToken')},
			filePath: path,
			name: name,
			formData: extra,
			success: function(res) {
				...
				try {
					...
					data = JSON.parse(res.data).content.results;
				} catch (e) {
					throw (e, data)
				}
